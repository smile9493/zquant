...
async fn setup_test_db() -> Result<PgPool> {
    ...
    // Clean up tables
    sqlx::query("TRUNCATE TABLE jobs, jobs_idempotency CASCADE").execute(&pool).await?;
...
async fn claim_skips_stop_requested() -> Result<()> {
    ...
    store.request_stop(&_job.job_id, Some("test stop".to_string())).await?;

    let claimed = store.claim_jobs(
        "executor_1",
        60000,
        10,
        &["test_job".to_string()],
...
async fn heartbeat_requires_matching_lease_version() -> Result<()> {
    ...
    let success = store.heartbeat_job(
        &claimed_job.job_id,
        "executor_1",
    ...
    let failure = store.heartbeat_job(
        &claimed_job.job_id,
        "executor_1",
        999,
        60000,
...
async fn finalize_requires_matching_lease_version() -> Result<()> {
    ...
    // Wrong lease_version should fail
    let failure = store.finalize_job(
        &claimed[0].job_id,
        "executor_1",
        999,
        JobStatus::Done,
    ...
    // Correct lease_version should succeed
    let success = store.finalize_job(
        &claimed[0].job_id,
        "executor_1",
